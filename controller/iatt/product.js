...
async function getAllProductsWithDeleted(request, reply) {
  try {
    const data =
      await iattService.product.getAllProductsWithDeleted();
    return reply
      .status(statusCode.success)
      .send({ data: data, message: successMessage.index });
  } catch (err) {
...
async function createProduct(request, reply) {
  try {
    ...
    const check = await iattValidation.validate(
      product,
      iattValidation.ProductSchema.CreateProductSchema,
      reply
    );
...
async function updateProduct(request, reply) {
  try {
    ...
    const check = await iattValidation.validate(
      product,
      iattValidation.ProductSchema.UpdateProductSchema,
      reply
    );
    if (check === false) {
      return reply
